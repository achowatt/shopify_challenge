    ...
    render(){
        ...
        <section className="nomination-section">
            <h2 className="nominations">
                {!this.props.nominationList.length ? "Uh oh! You have not nominated a movie..." : this.props.fiveNominations ? "Thank you for nominating 5 movies!" : "My Nominations"}
            </h2>
            {!this.props.nominationList.length ? <PlaceHolderImg/> : " "}
            <div className="nominated-card-container">
                {showNominationList}
            </div>
            {this.props.nominationList.length && this.props.fiveNominations ? <ClearAllNominations onClear={this.props.onClear}/> : ""}
            {this.props.nominationList.length && this.props.fiveNominations ? (<><Balloons speed1="15s" speed2="10s" /> <Balloons speed1="18s" speed2="12s" /></>) : ""}
        </section>
        )
    }
}
